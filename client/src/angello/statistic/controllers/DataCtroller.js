angular
		...
		.controller(
				...
				function($routeParams, user, stories, $log) {
					...
					myUser.stories = myUser.getAssignedStories(myUser.userId,
							stories);
					...
					function pieChart() {
						...
						// if($("svg").length >= 1){ //if exist svg in html
						// source code return, else create --0320remove the
						// if/else for once stepping into function there existed
						// myUser.clear()
						// return;
						// }else{
						// d3.select("body").attr("align", "center");
						var svg = d3.select(".flag").attr("align", "center")// align
																			// svg
																			// to
																			// center
																			// and
						...
						// ------------------------------------2.div提示框,通过设置透明度（opacity属性）实现
						// 显示和隐藏
						// 添加提示框的div
						var tooltip = d3.select("body").append("div").attr(
								"class", ".tooltip") // 用于css设置类样式
						.attr("opacity", 0.0);
					...
					function barChart() {
						...
						var svg = d3.select(".flag").attr("align", "center")
								.append("svg").attr("width", width).attr(
										"height", height);

						var xAxisScale = d3.scale.ordinal().domain(
								[ 'To Do', 'In Progress', 'Code Review',
										'QA Review', 'Verified' ]) // here you
																	// can
																	// define a
																	// range
																	// varible
						...
						var yAxisScale = d3.scale.linear().domain(
								[ 0, d3.max(dataset) ]).range([ 300, 0 ]);

						var xAxis = d3.svg.axis().scale(xAxisScale).orient(
								"bottom");

						var yAxis = d3.svg.axis().scale(yAxisScale).orient(
								"left");

						var xScale = d3.scale.ordinal().domain(
								d3.range(dataset.length)).rangeRoundBands(
								[ 0, 500 ], 0.4);

						var yScale = d3.scale.linear().domain(
								[ 0, d3.max(dataset) ]).range([ 0, 300 ]);

						svg.selectAll("rect").data(dataset).enter().append(
						...
						}).attr("height", yScale).attr("fill", function(d, i) {
							return color(i);
						});

						svg.selectAll("text").data(dataset).enter().append(
						...
						}).attr("dy", 25).attr("text-anchor", "begin").attr(
								"font-size", 14).attr("fill", "white").text(
								function(d, i) {
									return d;
								});

						svg.append("g").attr("class", "axis").attr("transform",
								"translate(30,350)").attr("fill", "white")
								.call(xAxis);

						svg.append("g").attr("class", "axis").attr("transform",
								"translate(30,50)").attr("fill", "white").call(
								yAxis);
					}
					...
					myUser.statusArr = [ [ "To Do", toDo ],
							[ "In Progress", inProgress ],
							[ "Code Review", codeReview ],
							[ "QA Review", qaReview ], [ "Verified", verified ] ];
					myUser.existed = true;
				});
